**** Program: 		Footnote2_vaccine_prevalence_2014.sas       ****;
...
array stype [12] ANESTH eeg ekg labtest mammog MEDPRESC mri RCVVAC SONOGRAM THRTSWAB XRAYS   SURGPROC;
array xstype[12]  x_ANESTH x_eeg x_ekg x_labtest x_mammog x_MEDPRESC x_mri x_RCVVAC
                  x_SONOGRAM x_THRTSWAB x_XRAYS  x_SURGPROC;

do i  = 1 to 12;
  xstype[i] = stype[i];
  if stype[i] = 95 then xstype[i] = 3;
  if stype[i] <0 then xstype[i] = .;
end;
...
if drsplty = -1 then YY_specialty=.N;
 ...
 FORMAT ins_cat ins_cat_F. YY_specialty YY_speclty_F. rcvvac  Rcvvac_F.;
run;
        ...
        proc crosstab data=obv2014_15_ana_rev filetype=sas DESIGN=WR;
            nest varpsu varstr  / missunit;
       		...
       		weight perwt14_15f;
       		subgroup ins_cat   YY_specialty x_rcvvac;
       		levels    2    6  3;
    		tables YY_specialty*ins_cat*x_rcvvac;
			rformat ins_cat ins_cat_F.;
			rformat YY_specialty YY_speclty_F.;
			rformat x_rcvvac  Rcvvac_F.;
	  ...
	  output  nsum wsum rowper="Pct" serow="SE_PCT" 
        /filename=INS_14_15_vacine replace;   
       run;
