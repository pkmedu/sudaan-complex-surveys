...
data op_17;
 merge person_flu_17 (in=p)
       pufmeps.h197f (in=op keep=dupersid evntidx seetlkpv VSTCTGRY); 
       by dupersid;
...
data op_16;
  merge person_flu_16 (in=p)
        pufmeps.h188f (in=op keep=dupersid evntidx seetlkpv VSTCTGRY); 
       by dupersid;
...
data new.op_16_17;
  set op_16 op_17 indsname=source;
  Year=scan(source, -1, '_');
  perwtf_16_17 = perwtf/2;
run;
options nolabel;
proc means data=new.op_16_17; run;
proc freq data=new.op_16_17;
...
ods listing;
ods graphics off;
title 'For the Definition of the Stat Brief  - % of hospital outpatient visits - called by phone, combined data from 2016-2017';
proc surveymeans data=new.op_16_17 nops  mean stderr sum ;
  stratum varstr;
  cluster varpsu;
  weight perwtf_16_17;
  var  x_seetlkpv;
...
title 'For the Definition of the Stat Brief - % of hospital output visits - received immunization /shots, combined data from 2016-2017';
proc surveymeans data=new.op_16_17 nops  mean stderr sum ;
  stratum varstr;
  cluster varpsu;
  weight perwtf_16_17;
